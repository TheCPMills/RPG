...
{   public int itsFatiguePoints;
    public boolean isRadioactive = false;
    public Good()
    {   
    }
    
    public Good(String name, Archetype archetype, Attribute[] attributes, Skill[] skills)
    {   super(name, archetype, attributes, skills);
    }
    
    public void attack(Item item, UniversalCharacter c)
    {   if(isAlive && !isRadioactive)
    ...
    {   if(isAlive && !isRadioactive)
        {   if(item.getClass().toString().compareTo("class tools.Spell") == 0 || item.getClass().toString().compareTo("class tools.Ritual") == 0)
            ...
            else if(item.getClass().toString().compareTo("class tools.Throwing") == 0 || item.getClass().toString().compareTo("class tools.Potion") == 0 || item.getClass().toString().equals("class tools.ElectromagneticRadiationRemovalKit"))
    ...
    {   System.out.println(itsName + " Used " + s.getItsName());
        ...
        if(r)
        {   String q = c.itsName + " is Inflicted With " + s.effect();
            if(!Utilities.inList(c.itsConditions, Condition.Stunned) && s.effect() != Condition.Healing)
            {   c.addCondition(s.effect().toString());
            }
            else if(s.effect() == Condition.Healing)
    ...
    {   System.out.println(itsName + " Used " + e.getItsName());
        ...
        {   int i = ((ElectromagneticRadiationRemovalKit) (e)).removeRadiation();
            while(i > -1)
            {   ((GoodHitPoints) (itsHitPoints)).loseRadiation();
                i--;
            }
    ...
    {   if(isAlive && !isRadioactive)
        {   int b = itsHitPoints.size() - ((GoodHitPoints) (itsHitPoints)).itsHealthPoints - ((GoodHitPoints) (itsHitPoints)).itsRadioactivePoints;
            if(i > b)
            {   i = b;
            }
    ...
    {   if(isAlive && !isRadioactive)
        {   int b = ((GoodHitPoints) (itsHitPoints)).itsHealthPoints;
            if(i > b)
            {   i = b;
            }
            for(int count = 0; count < i; count++)
            ...
            if(((GoodHitPoints) (itsHitPoints)).itsHealthPoints == 0)
    ...
    public void irradiate(int i)
    {   if(isAlive && !isRadioactive)
        {   for(int j = itsHitPoints.size() - 1, count = 0; count < i; j--, count++)
            {   ((GoodHitPoints) (itsHitPoints)).addRadiation();
            }
            if(((GoodHitPoints) (itsHitPoints)).itsRadioactivePoints >= itsHitPoints.size() / 2)
            {   addCondition("Radioactivity");
            }
        }
    }

    public abstract Evil evilRevival();

    public void becomeFatigued()
    {   if(isAlive && !isRadioactive)
        {   if(itsFatiguePoints >= 100)
            {   itsFatiguePoints = 100;
                itsConditions.add(Condition.Fatigue);
            }
        }
    }
    ...
    {   if(isAlive && !isRadioactive)
        {   if(itsConditions.indexOf(Condition.Insomnia) != -1)
            {   System.out.println(itsName + " Cannot Sleep\nThey Have Insomnia");
            }
            else
            {   itsFatiguePoints -= i;
                if(itsFatiguePoints < 0)
                {   itsFatiguePoints = 0;
                }
                itsConditions.remove(Condition.Fatigue);
            }
        }
    }
    ...
    {   if(isAlive && !isRadioactive)
        {   int j = -1;
            for(int i = 0; i < itsEquipment.size(); i++)
            {   if(itsEquipment.get(i).getClass().toString().equals("class tools.Ration"))
                {   j = i;
                }
            }
            if(j == -1)
            {  System.out.println(itsName + " Has Nothing to Eat or Drink");
            }
            else
            {   if(itsConditions.indexOf(Condition.Hunger) != -1)
                {   itsConditions.remove(Condition.Hunger);
                }
                if(itsConditions.indexOf(Condition.Insomnia) != -1)
                {   itsConditions.remove(Condition.Insomnia);
                }
                if(itsConditions.indexOf(Condition.Poison) != -1)
                {   itsConditions.remove(Condition.Poison);
                }
                if(itsConditions.indexOf(Condition.Paralysis) != -1)
                {   itsConditions.remove(Condition.Paralysis);
                }
                if(itsConditions.indexOf(Condition.Blindness) != -1)
                {   itsConditions.remove(Condition.Blindness);
                }
                if(itsConditions.indexOf(Condition.Anosmia) != -1)
                {   itsConditions.remove(Condition.Anosmia);
                }
                if(itsConditions.indexOf(Condition.Ageusia) != -1)
                {   itsConditions.remove(Condition.Ageusia);
                }
                if(itsConditions.indexOf(Condition.Deafness) != -1)
                {   itsConditions.remove(Condition.Deafness);
                }
                if(itsConditions.indexOf(Condition.Slowness) != -1)
                {   itsConditions.remove(Condition.Slowness);
                }
                if(itsConditions.indexOf(Condition.Akathisia) != -1)
                {   itsConditions.remove(Condition.Akathisia);
                }
                if(itsConditions.indexOf(Condition.Aphasia) != -1)
                {   itsConditions.remove(Condition.Aphasia);
                }
                if(itsConditions.indexOf(Condition.Asthenia) != -1)
                {   itsConditions.remove(Condition.Asthenia);
                }
                addHitPoints(1);
            }
        }
